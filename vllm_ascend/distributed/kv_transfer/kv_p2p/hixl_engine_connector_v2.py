...
_HIXL_ENGINE_BACKENDS = frozenset({_HIXL_ENGINE_BACKEND_CS, _HIXL_ENGINE_BACKEND_COMM})
...
_LISTEN_PORT_OFFSET = 10000
_DEFAULT_LINK_TIMEOUT_MS = 5000
_DEFAULT_TRANSFER_TIMEOUT_MS = 60_000
...
_ORPHAN_DRAIN_MS = 30_000
_TERMINAL_FAIL = frozenset({"FAILED", "TIMEOUT"})
...
def _flatten_grc_protocol_desc(options: dict[str, str]) -> bool:
    ...
    options[_HIXL_OPTION_GLOBAL_RESOURCE_CONFIG] = json.dumps(obj, separators=(",", ":"))
    return True
...
def _normalize_hixl_engine_options(raw: Any) -> dict[str, str]:
    ...
    if not isinstance(raw, dict):
        raise ValueError(f"hixl_engine.options must be a dict, got {type(raw).__name__}.")
    out: dict[str, str] = {}
...
def _apply_hixl_engine_backend_options(backend: str, options: dict[str, str]) -> tuple[dict[str, str], str]:
    out = dict(options)
    ...
    if backend == _HIXL_ENGINE_BACKEND_COMM:
        ...
        if stripped:
            logger.warning("hixl_engine.backend=comm stripped CS selector(s) %s.", ",".join(stripped))
...
def _parse_hixl_engine_v2_extra(
    ...
    if raw_backend is None or (isinstance(raw_backend, str) and not raw_backend.strip()):
    ...
    if listen_port_base is None:
        listen_port_base = int(kv_port) + _LISTEN_PORT_OFFSET
    ...
    return (
        ...
        backend_requested,
        injected,
        int(cfg.get("link_timeout_ms", _DEFAULT_LINK_TIMEOUT_MS)),
        int(cfg.get("transfer_timeout_ms", _DEFAULT_TRANSFER_TIMEOUT_MS)),
    )
...
class _Hixl:
    ...
    def sync_read(
        ...
        if not (len(src_list) == len(dst_list) == len(length_list)):
            raise ValueError(
                f"src/dst/length lists must have the same length, got {len(src_list)}, {len(dst_list)}, {len(length_list)}"
            )
        if not src_list:
            return 0
        self._reap_orphans()
        ...
        descs = [
            self._hixl_mod.TransferOpDesc(local_addr=int(local), remote_addr=int(remote), len=int(nbytes))
            for local, remote, nbytes in zip(src_list, dst_list, length_list, strict=True)
        ]
        with self._lock:
            self._check_live()
    ...
    def _await_transfer(self, handle: int, remote_engine: str) -> None:
        ...
        we keep draining for _ORPHAN_DRAIN_MS and only then abandon the handle,
        ...
        hard_deadline = soft_deadline + _ORPHAN_DRAIN_MS / 1000.0
        overdue = False
        while True:
            with self._lock:
                self._check_live()
            ...
            if overdue and now >= hard_deadline:
                ...
                raise RuntimeError(
                    ...
                    f"{self.transfer_timeout_ms + _ORPHAN_DRAIN_MS} ms"
                )
            time.sleep(0.001)
...
class KVCacheRecvingThread(threading.Thread):
    ...
    def _transfer_kv_cache_all_groups(self, req_meta: dict[str, Any]):
        ...
        transfer_bandwidth_gbs = transfer_bytes / transfer_elapsed_s / 1e9 if transfer_elapsed_s > 0 else 0.0
        logger.info(
...
class MooncakeConnectorWorker:
    ...
    def _get_kernel_block_ids(self, layer_indices, meta, group_idx, group_spec):
        ...
        if num_external_tokens > 0:
            ...
            needed_kernels = cdiv(tokens_to_cover, remote_kernel_token_size) - remote_start_idx
            if needed_kernels > 0:
